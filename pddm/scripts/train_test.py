    ...
    with tf.Session(config=get_gpu_config(args.use_gpu, args.gpu_frac)) as sess:
        ...
        if continue_run==-1:
            ...
            if n_KER:
                ...
                Data = rollouts_trainRand
                rollouts_trainRand =[]
                for data in Data:
                    original_ka_episodes = KER.ker_process(data.states, data.actions, data.g, data.ag)
                    for episode in original_ka_episodes:
                        rollout = Rollout(observations=np.array(episode[0]), actions=np.array(episode[1]), desired_goals=np.array(episode[2]), achieved_goals=np.array(episode[3]))
        ...
        while counter < num_iters:
            ...
            # print(np.mean(np.array(list_rewards)))
            ...
            numData_train_rand += get_num_data(rollouts_rand)
